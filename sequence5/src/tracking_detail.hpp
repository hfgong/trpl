...
template <typename Float>
void combine_ground_score(int tt,
			  vector<candidate_array<Float> > const& cand_array,
			  ...
			  geometric_info_t const& gi)
{
	...
	imbd_x <<= x0, x1, x1, x0;
	imbd_y <<= y0, y0, y1, y1;
	apply_homography(gi.img2grd(tt, cam), imbd_x, imbd_y, grbd_x(cam), grbd_y(cam));
	...
	for(int yy=gy0; yy<gy1; ++yy)
	{
	    ...
	    apply_homography(gi.grd2img(tt, cam), grx, gry, imx, imy);
