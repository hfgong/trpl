		       ...
		       std::map<STR, STR>& settings) 
{
    ...
    {
	STR& s= string_list[ii];
	boost::trim(s);

	if(s.size()==0) continue;

	std::vector<STR> kvpair;
...
template <class Mat>
bool write_text_array1d(const std::string& file, Mat const& mat)
{
    typedef array1d_traits<Mat> tr;
    typedef typename tr::value_type T;
    ...
    for(std::size_t jj=0; jj<tr::size(mat); ++jj)
    {
	fout<<tr::ref(mat, jj)<<" ";
    }
    fout<<std::endl;
...
template <class Mat>
bool write_text_array2d(const std::string& file, Mat const& mat)
{
    typedef array2d_traits<Mat> tr;
    typedef typename tr::value_type T;
    std::ofstream fout(file.c_str());
