...
#if 0
	array<float, 2> da = {goal[0]-pos[0], goal[1]-pos[1]};
	float div = std::sqrt(da[0]*da[0]+da[1]*da[1])+1e-6;
	array<float, 2> vid = {da[0]*speed/div, da[1]*speed/div};
	std::cout<<"goal-pos="<<da[0]<<", "<<da[1]<<std::endl;
	std::cout<<"videal="<<vid[0]<<", "<<vid[1]<<std::endl;
	std::cout<<"vsolut="<<vel[0]<<", "<<vel[1]<<std::endl;
#endif
    }
