		   ...
		   vector<vector<int> > const& trj_index)
{
	...
	for(int tt=0; tt<T; ++tt)
	{
	    if(tt>=trj.startt && tt<=trj.endt) continue;
	    for(int cam=0; cam<Ncam; ++cam)
	    {
	...
	for(int tt=trj.startt; tt<=trj.endt; ++tt)
	{
		...
		std::string name = ds.output+
		    str(format("grd_trj_nopl_%03d_%03d.png")%nn%tt);
		...
		for(int cc=0; cc<A::size1(vis); ++cc)
		{
			...
			for(int xx=0; xx<A::size3(vis); ++xx)
			{
				...
				A::ref(vis, cc, yy, xx) = A::ref(vis, cc, yy, xx);
			}
		    }
		}
		for(int mm=0; mm<trj_list.size(); ++mm)
...
int main(int argc, char* argv[])
{
    ...
    write_bounding_boxes(merged_trj_list, ds.workspace+"results_app.txt");
