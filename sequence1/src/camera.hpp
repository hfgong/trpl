				 ...
				 ground_lim_t& ground_lim)
{
    ...
    ix <<= img_x(0), img_x(4), img_x(6), img_x(7);
    iy <<= img_y(0), img_y(4), img_y(6), img_y(7);
    gx <<= grd_x(0), grd_x(4), grd_x(6), grd_x(7);
    gy <<= grd_y(0), grd_y(4), grd_y(6), grd_y(7);
...
//% Convert Left side ground polygon to Right side ground polygon
    ...
    img_x = row(poly2dr, 0);
    img_y = row(poly2dr, 1);
    ix(0) = img_x(0); ix(1) = img_x(4); ix(2) = img_x(6); ix(3) = img_x(7);
    iy(0) = img_y(0); iy(1) = img_y(4); iy(2) = img_y(6); iy(3) = img_y(7);
