...
//
	...
	for(int tt=T0; tt<T1; ++tt)
	{
	    ...
	    it = pid_gt.find(tt) ;
	    if(it != pid_gt.end())
		add_patch_to_tile(gt2rst_tile, patches_gt(0, it->second), 0, tt-T0);
	    it = pid_pl.find(tt);
	    if(it != pid_pl.end())
		add_patch_to_tile(gt2rst_tile, patches_rst_plan(0, it->second), 1, tt-T0);
	    it = pid_ap.find(tt);
	    if(it != pid_ap.end())
		add_patch_to_tile(gt2rst_tile, patches_rst_app(0, it->second), 2, tt-T0);
	}
