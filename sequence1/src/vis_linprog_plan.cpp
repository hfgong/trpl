		     ...
		     geometric_info_t const& gi)
{
	...
	//			10, yellow);
	int gx = static_cast<int>(gi.goal_ground(0, gg));
	int gy = static_cast<int>(gi.goal_ground(1, gg));
	unsigned char gray[]={128, 128, 128};
	ground_fig.draw_rectangle(gx-8, gy-8, gx+8, gy+8, gray, 1, ~0U);
	ground_fig.draw_rectangle(gx-8, gy-8, gx+8, gy+8, gray);
    }
...
int main(int argc, char* argv[])
{
    ...
    write_bounding_boxes(merged_trj_list, ds.workspace+"results_plan.txt");
